...
export default function AddItemPage() {
  ...
  const applyReviewItemToForm = (item: ReviewDetectedItem) => {
    const matchedInventoryName = getMatchedInventoryName(item)
    const directMatch = item.matchedProductId ? products.find((p) => p.id === item.matchedProductId) || null : null
    const directMatchAligned =
      directMatch && normalizeNameForMatch(directMatch.name) === normalizeNameForMatch(matchedInventoryName)
        ? directMatch
        : null
    const nameMatch = findBestProductByName(matchedInventoryName)
    const chosenProduct = directMatchAligned || nameMatch
      ...
      for (const item of reviewItems) {
        ...
        const directMatchAligned =
          directMatch && normalizeNameForMatch(directMatch.name) === normalizeNameForMatch(matchedInventoryName)
            ...
            : null
        const nameMatch = findBestProductByName(matchedInventoryName)
        let chosenProduct = directMatchAligned || nameMatch

        if (!chosenProduct) {
          const primaryName = matchedInventoryName
